...
def getHistoryPriceData(area_list):
	...
	for area in area_list:
		...
		#File path
		filePath = "./data/" + area + "-fluctuate.csv"
		#File exists?
		...
		if file:
		    ...
		    for row in rows:
		        rData = row.split(",")
		        if rData[0] not in yearsMonths:
		            yearsMonths.add(rData[0])
		    #Close read file
		    rfile.close()

		#Write information to file
		wfile = open(filePath, "a")
		#File titles (Only be execute once when file is not exist)
		if not file:
		    wfile.write(table_titles)
		...
		html = requests.get("https://"+area+".listing.ca/real-estate-price-history.htm")
				...
				for index in range(0, titles_length):
					...
					for tr in trs[1:]:
						...
						for td in tds:
							...
							if tds.index(td) == 0:
								yearMonth = titles[index].text[0:4]+'/'+td.text
								line += yearMonth + ","
								# print(yearMonth, end="\t")
							else:
								line += td.text.replace(",","") + ","
								# print(td.text, end="\t")
						if yearMonth not in yearsMonths:
							line = line[:-1] + "\n"
							wfile.write(line)
						# print()
...
area_list = ['barrie', 'brampton', 'burlington', 'caledon', 'clarington', 
	'hamilton', 'innisfil', 'kawartha-lakes', 'king', 'markham', 'milton', 
	'mississauga', 'newmarket', 'oakville', 'oshawa', 'richmond-hill', 'toronto', 'vaughan', 'whitchurch-stouffville']
